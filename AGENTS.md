...
- `app/setup` MySQL 配置页 + 设置弹窗（数据库/常规两个 tab）；`app/(analysis)` 分析页面组（dashboard / players 玩家中心 / signin / authme）
- `app/api/mysql/*` 配置链路；`app/api/<插件id>/*` 各插件只读分析接口；`app/api/players/*` 玩家中心聚合接口（列表/详情/时间线）
- `lib/server/player-registry.ts` 跨插件玩家目录（UUID 主键 + AuthMe 名称桥接，30s 缓存）；`lib/server/unified-players.ts` 玩家中心聚合查询
- `lib/common/avatar.ts` mc-heads.net 头像 URL；`lib/common/unified.ts` 玩家中心共享类型（客户端/服务端共用）
