...
import {
  TITLE_PLAYER_DEFAULT_ORDER,
  type TitlePlayerItem,
  type TitlePlayerSortField,
...
export default function TitlePlayersPage() {
  ...
  const [sort, setSort] = useState<TitlePlayerSortField>("count");
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        const result = await fetchJson<Paginated<TitlePlayerItem>>(
          `/api/playertitle/players?${params}`,
        );
  ...
  function handleSort(field: TitlePlayerSortField) {
    const next = toggleSort(
      { field: sort, order },
      field,
      TITLE_PLAYER_DEFAULT_ORDER[field],
    );
    setSort(next.field);
    setOrder(next.order);
    setPage(1);
        ...
        {error ? (
          ...
          >
            <Table>
              <TableHeader>
                <TableRow>
                  <SortableHead<TitlePlayerSortField>
                    label="玩家"
                    field="name"
                    sort={sort}
                    order={order}
